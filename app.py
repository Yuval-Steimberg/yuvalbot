...
Runs Flask dashboard + background job scanner scheduler.
"""
...
SCAN_STATUS = {"last": "Never", "running": False, "next": ""}
...
def login():
    ...
    if request.method == "POST":
        pwd = os.environ.get("DASHBOARD_PASSWORD","yuval2026")
...
button{{width:100%;background:#00ff88;color:#000;border:none;padding:12px;border-radius:5px;font-family:inherit;font-weight:700;font-size:13px;cursor:pointer;letter-spacing:.04em}}
...
.btn-g{background:var(--g);color:#000}.btn-g:hover{opacity:.85;transform:translateY(-1px)}
.btn-o{background:transparent;border:1px solid var(--g);color:var(--g)}.btn-o:hover{background:rgba(0,255,136,.08)}
.btn-y{background:transparent;border:1px solid var(--y);color:var(--y)}.btn-y:hover{background:rgba(255,228,77,.08)}
.btn-r{background:transparent;border:1px solid var(--r);color:var(--r)}.btn-r:hover{background:rgba(255,85,85,.08)}
...
.status-bar{background:#0d0d1a;border-bottom:1px solid var(--b);padding:6px 24px;font-size:10px;color:var(--d);display:flex;gap:16px}
.status-bar span{color:var(--t)}
@media(max-width:700px){.stats{grid-template-columns:repeat(2,1fr)}.table-w{padding:0 12px 40px}nav{padding:11px 14px}.filters{padding:10px 14px}}
</style></head>
<body>
<nav>
  <div class="logo">YUVAL<span>.BOT</span></div>
  <div class="nav-r">
    ...
    <button class="btn btn-g" onclick="doScan()">⟳ Scan Now</button>
    <a href="/logout"><button class="btn btn-r btn-sm">Logout</button></a>
  </div>
</nav>
<div class="status-bar">
  <div>Last scan: <span id="lastScan">–</span></div>
  <div>Next scan: <span id="nextScan">–</span></div>
  <div>Sources: <span style="color:var(--g)">Indeed · LinkedIn · Glassdoor · Google · ZipRecruiter</span></div>
</div>
...
<div class="filters">
  ...
  <button class="fb" onclick="filt('glassdoor',this)">Glassdoor</button>
  <button class="fb" onclick="filt('google',this)">Google</button>
  <button class="fb" onclick="filt('zip_recruiter',this)">ZipRecruiter</button>
</div>
<div class="table-w">
  <table><thead><tr>
    <th>Score</th><th>Job</th><th>Source</th><th>Found</th><th>Status</th><th>Actions</th>
  </tr></thead>
  <tbody id="tbody"><tr><td colspan="6" style="text-align:center;color:var(--d);padding:40px">Loading...</td></tr></tbody>
  </table>
</div>
...
async function load(){
  ...
  const st=await fetch('/api/status-info'); const si=await st.json();
...
function render(){
  const src=['indeed','linkedin','glassdoor','google','zip_recruiter'];
  const f=cur==='all'?jobs:src.includes(cur)?jobs.filter(j=>j.source===cur):jobs.filter(j=>j.status===cur);
  if(!f.length){document.getElementById('tbody').innerHTML='<tr><td colspan="6" style="text-align:center;color:var(--d);padding:40px">No jobs</td></tr>';return;}
  document.getElementById('tbody').innerHTML=f.map(j=>{
    ...
    return `<tr>
      ...
      <td style="color:var(--d);white-space:nowrap;font-size:11px">${dt}</td>
...
function show(id){
  ...
  document.getElementById('cv-t').textContent=j.tailored_cv||'Not generated yet — click Scan Now.';
...
function swTab(n,el){
  ...
  document.getElementById('tab-'+n).classList.add('a');
...
async function mark(id,st){
  ...
  stats(); render(); toast(st==='Applied'?'✓ Marked Applied!':'Skipped');
}
async function doScan(){
  const b=event.target; b.disabled=true; b.innerHTML='<span class="spin"></span>Scanning...';
  toast('🔍 Scanning all sources — ~3 minutes');
  try{
    await fetch('/api/scan',{method:'POST'});
    await new Promise(r=>setTimeout(r,180000)); // wait 3 min
    await load(); toast('✅ Scan complete!');
  }catch(e){toast('Check logs for errors');}
...
if __name__ == "__main__":
    ...
    log.info(f"🔑 Password: {os.environ.get('DASHBOARD_PASSWORD','yuval2026')}")
    try:
